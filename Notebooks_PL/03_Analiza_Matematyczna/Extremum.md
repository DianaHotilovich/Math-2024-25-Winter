...
## Упрощение функции  
Числитель $ x^2 + 2x + 1 $ можно представить как $ (x + 1)^2 $. Тогда:  

$$
f(x) = \frac{(x + 1)^2}{x - 1}.
$$
Uprościliśmy licznik funkcji.  

### Производная функции / Obliczymy pochodną funkcji.  
...
Подставим в формулу:  / podstawiamy
...
Итак, производная:  / pochodna funkcji
...
### Нахождение критических точек  / Aby znaleźć punkty krytyczne, przyrównujemy pochodną do zera:  
Приравняем $ f'(x) = 0 $. Числитель должен быть равен нулю:  
...
Знак $ f'(x) $ определяется числителем $ (x - 3)(x + 1) $, так как знаменатель $ (x - 1)^2 > 0 $ при $ x \neq 1 $. Разделим область определения на интервалы:  
Znak $ f'(x) $ jest określany przez licznik $ (x - 3)(x + 1) $, ponieważ mianownik $ (x - 1)^2 > 0 $ dla $ x \neq 1 $. Podzielmy dziedzinę funkcji na przedziały:
...
- przy  $ (-\infty, -1) $: $ f'(x) > 0 $, функция возрастает.  rośnie
- przy $ (-1, 1) $: $ f'(x) < 0 $, функция убывает. maleje 
- przy $ (1, 3) $: $ f'(x) < 0 $, функция убывает.  
- przy $ (3, +\infty) $: $ f'(x) > 0 $, функция возрастает.  

### Выводы: / Wynik
1. $ x = -1 $:  $ f'(x) > 0 \to f'(x) < 0 $. Это **точка максимума**.  
2. $ x = 3 $:  $ f'(x) < 0 \to f'(x) > 0 $. Это **точка минимума**.  

### Значения функции в критических точках  / Wartości funkcji w punktach krytycznych
